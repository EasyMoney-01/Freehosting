...
app.use("/api/services", authMiddleware(jwtSecret), servicesRouter);
...
app.all("/s/:username/:servicename/*", (req, res) => {
  const { username, servicename } = req.params;
  const rest = req.params[0] || "";
  db.get("SELECT id FROM users WHERE username = ?", [username], (e1, user) => {
    if (e1 || !user) return res.status(404).send("Not found");
    db.get("SELECT port, status FROM services WHERE owner_id = ? AND name = ?", [user.id, servicename], (e2, svc) => {
      if (e2 || !svc || !svc.port || svc.status !== "running") return res.status(404).send("Not found");
      const opts = {
        hostname: "127.0.0.1",
        port: svc.port,
        path: "/" + rest,
      ...
      proxyReq.on("error", () => res.status(502).send("Bad gateway"));
